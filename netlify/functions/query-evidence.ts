...
import { askEvidenceModel, embedText } from '@/lib/openai';
...
export const handler = async (event: { httpMethod: string; body: string | null }) => {
  ...
  const retrievedContext = toRetrievedContext(typedChunks);
  ...
  const modelResponse =
    typedChunks.length === 0
      ...
      : await askEvidenceModel(parsed.data.query, retrievedContext);

  await supabase.from('audit_logs').insert({
    tenant_id: tenantId,
    user_id: null,
    action_type: 'evidence_answer',
